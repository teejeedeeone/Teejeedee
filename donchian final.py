...
3s
pip install ccxt
Requirement already satisfied: ccxt in /usr/local/lib/python3.11/dist-packages (4.4.69)
...
def get_perpetual_symbols():
    try:
        ...
        # Filter markets for perpetual futures
        perpetual_symbols = [
            market['symbol'] for market in exchange.markets.values()
            if market['type'] == 'swap' and not market['spot']
        ]

        # Print perpetual symbols in the specified format
        print(perpetual_symbols)
